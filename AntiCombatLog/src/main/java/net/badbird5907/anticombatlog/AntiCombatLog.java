...
public final class AntiCombatLog extends JavaPlugin { //TODO config editor in game
    ...
    public static void tag(Player victim, Player attacker) {
        tag(victim, attacker, true, ConfigValues.getCombatTagSeconds());
    }

    public static void tag(Player victim, Player attacker, boolean sendMessages, int timeout) {
        ...
        sendMessageVictim = sendMessageVictim && sendMessages;
        sendMessageAttacker = sendMessageAttacker && sendMessages;
        inCombatTag.put(victim.getUniqueId(), timeout);
        inCombatTag.put(attacker.getUniqueId(), timeout);
        if (ConfigValues.getCombatTaggedMessage() != null) {
            if (sendMessageVictim)
                victim.sendMessage(StringUtils.format(ConfigValues.getCombatTaggedMessage(), timeout + ""));
            if (sendMessageAttacker && !victim.equals(attacker))
                attacker.sendMessage(StringUtils.format(ConfigValues.getCombatTaggedMessage(), timeout + ""));
        }
    ...
    public static void silent_tag(UUID uuid, int timeout) {
        Player player = Bukkit.getPlayer(uuid);
        if (player != null && player.getGameMode() == GameMode.CREATIVE)
            return;
        inCombatTag.put(uuid, timeout);
        if (player != null && ConfigValues.isDisableFly()) {
