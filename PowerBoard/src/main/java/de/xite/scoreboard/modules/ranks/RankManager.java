...
public class RankManager {
		...
		for(Player all : Bukkit.getOnlinePlayers()) {
			if(all != p) {
				...
				if(teams != null) {
					...
					//String prefix = getCurrentPrefix(p, teams.getPrefix());
	...
	public static String getCurrentPrefix(Player p, String oldPrefix) {
		try {
			BankSys api = BankSys.getInstance();
			if (api != null && api.isInRaid(p.getUniqueId())) {
				//Bukkit.getLogger().log(Level.SEVERE, p.getDisplayName() + " is in raid");
				String newPrefix = api.getCurrentPrefix(p.getUniqueId());
				if (newPrefix != null) {
					//Bukkit.getLogger().log(Level.SEVERE, "hijacking prefix to " + newPrefix);
					return newPrefix;
				}
			}
		...
		//Bukkit.getLogger().log(Level.SEVERE, p.getDisplayName() + " is not in raid");
		return oldPrefix;
	}
	...
	public static void setPrefixSuffix(Player p, Team t, String prefix, String suffix, String playerListName) {
		...
		if(playerListName != null) {
			if(!usePrefixSuffixForPlayerListName) {
				//Bukkit.getLogger().log(Level.SEVERE, "Setting rank for " + p.getDisplayName() + " null");
				t.setPrefix("");
				t.setSuffix("");
			}
